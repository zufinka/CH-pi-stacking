...
void Main()
{
	...
	var files = Directory.GetFiles("motivy/C3C4/TYR/all2/vodiky/ligand"); //seznam PDB motivů s vodíky
	...
	*/								
	...
	foreach (var e in files)
	{
		...
		{	
			...
			{	
				...
				if ((distance < minDistance) && (sorted.ElementAt(i).First().PdbResidueName() != "TYR") )//teoreticky můžu mít v sorted víc aromatických residuí než jedno
				{
    ...
    File.WriteAllLines("TYR_ligand.csv", zapisSet);
	...
	int counter = 1;
	using (TextWriter writer = File.CreateText("TYR_ligand_pdb.csv"))
	{
	...
	using (TextWriter wr = File.CreateText("TYR_ligand_ligands.csv"))
	{
