...
void Main()
{
	...
	*/								
	...
	foreach (var e in files)
	{
         ...
         */
		if (sorted.Count > 6) //závisí na počtu atomů arom. residua, které ukládám
		{	
			...
			for (int i = 6; i < sorted.Count; i++)//hledám nejbližší atom cukru = procházím až atomy cukru !!!zkontrolovat kolik atomů aromatické amk je před tím!!!
			{	
				...
				if ((distance < minDistance) && (sorted.ElementAt(i).First().PdbResidueName() != "PHE") )//teoreticky můžu mít v sorted víc aromatických residuí než jedno
				{
            ...
            */
			...
			Plane3D rovina = Plane3D.FromPoints(sorted.ElementAt(3).First().Position, sorted.ElementAt(4).First().Position, aromCenter);
			...
			if ((minDistance < 4.5) && (filterAngle < 45)) //filtr pro výběr motivů
			{
				...
				var path = new StringBuilder("motivy/C3C4/PHE/5_11_2017/ligand/");
